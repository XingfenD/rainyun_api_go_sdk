...
**当前最需要的是 API 响应结构体定义。** SDK 中仍有大量响应字段/端点因上游未公开结构而以 `Data any` 透传，并标记了 `TODO: 结构未公开,实测后补强类型`（可在代码中搜索该标记）。

[`docs/PROGRESS.md`](PROGRESS.md) 的"响应类型"列标注了每个端点的处理方式：
- **typed**：强类型响应，字段已根据实测或文档定义
- **passthrough**：`Data any` 透传，响应结构未公开或无法安全探测

如果您有雨云账号并能调用对应接口并获取有意义的响应，欢迎提交 PR 补全：

1. 用您的 API key 实测接口，获取真实响应体（在调用前请务必确认该接口为只读，仅在确有需要时调用会**修改资源**的敏感操作）。
